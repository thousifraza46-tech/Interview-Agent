...
from audio_recorder import save_recorded_audio, cleanup_audio_file
try:
    from audio_recorder_streamlit import audio_recorder
    AUDIO_RECORDER_AVAILABLE = True
except ImportError:
    AUDIO_RECORDER_AVAILABLE = False
    print("Warning: audio-recorder-streamlit not installed. Voice recording will be disabled.")
        ...
        if st.session_state.current_question and not st.session_state.get('timer_expired', False):
            ...
            with tab2:
                if not AUDIO_RECORDER_AVAILABLE:
                    st.warning("Voice recording is not available. Please install audio-recorder-streamlit.")
                    st.info("You can still upload an audio file below:")
                    audio_file = st.file_uploader(
                        "Upload audio file (mp3, wav, m4a)",
                        type=['mp3', 'wav', 'm4a'],
                        key=f"audio_upload_{st.session_state.current_question_num}"
                    )
                    ...
                    if audio_file:
                        ...
                        if st.button("Transcribe & Submit", type="primary", key="submit_voice_upload"):
                    ...
                    st.info("Click the microphone button below to record your answer (browser-based, no server microphone needed)")
                    ...
                    audio_bytes = audio_recorder(
                        ...
                        icon_size="3x",
                        key=f"audio_recorder_{st.session_state.current_question_num}"
                    )
                    ...
                    if audio_bytes:
                        ...
                        audio_path = save_recorded_audio(
                            audio_bytes,
                            filename=f"question_{st.session_state.current_question_num + 1}.wav"
                        )
                        
                        # Play back the recording
                        st.audio(audio_bytes, format="audio/wav")
                        
                        if st.button("Transcribe & Submit", type="primary", key="submit_voice_recorded"):
                            with st.spinner("Transcribing audio..."):
                                try:
