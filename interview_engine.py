...
from config import OPENAI_API_KEY, GEMINI_API_KEY, AI_PROVIDER, OPENAI_MODEL, GEMINI_MODEL
import json
...
Make the question {"challenging and require deep understanding" if level == "Hard" else "moderately challenging" if level == "Medium" else "foundational and clear"}."""
    ...
    try:
        ...
        fallback_questions = QUESTION_BANK.get(role, QUESTION_BANK["Python Developer"])
        level_questions = fallback_questions.get(level, fallback_questions["Easy"])
        return level_questions[question_number % len(level_questions)]
...
def generate_question(role: str, level: str, question_number: int = 0, include_hr: bool = True, use_ai: bool = True) -> Dict:
    ...
    if use_ai:
        try:
            return generate_ai_question(role, level, question_number, is_hr_question)
        except Exception as e:
